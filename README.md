...
Wholegrain Studios is the central account service. Pips, Tadoo, Nom, and Splob redirect players to `/accounts/link` with `game`, `gameAccountId`, and `returnTo` query parameters. Wholegrain handles login, then calls the app server-to-server through `/.netlify/functions/link-game-account`.
...
- `SPLOB_LINK_ENDPOINT`: the protected Splob backend endpoint. Keep the actual value only in Netlify, not in committed docs or client-side code.
- `SPLOB_RETURN_ORIGINS`: comma-separated origins that the link page may redirect back to. Keep the actual values only in Netlify if the variable is marked secret.
...
### Required Splob Netlify settings

Set these environment variables on the Splob Netlify site:
